...
int main()
 {
	...
	int server_bind_id;
	server_bind_id = bind( server_sock_id , (struct sockaddr*)&server, sizeof(server) );
    ...
    */
    ...
    int    Shared_mem_ID, status;
	int    *total_client;
    Shared_mem_ID = shmget(IPC_PRIVATE, sizeof(int), IPC_CREAT | 0666);
    total_client = (int *) shmat(Shared_mem_ID, NULL, 0);
 
    while(1)
	{ 
        
        int new_Socket = accept(server_sock_id, (struct sockaddr*)&newAddr, &addr_size);
        if(newSocket<0)
        {
            exit(1);
        }
        client_id = new_Socket;
        ...
        if((childpid = fork()) == 0)
        {
            ...
            if (*total_client == 6)
            {
				char msg[] = "Max clients connected, try again later\n:";
            ...
            else
            {
                ...
                bzero(message_sent,256);
                snprintf(message_sent,200,"SERVER IS READY TO PROCESS REQUEST:\n");
                ...
                { 
                    ...
                    printf( "\nWaiting for command/connection request \n\n");
                    fflush(stdin);
                    fflush(stdout);
                    while (1)
                    {
                    ...
                    }else if(!strcmp(command,"exit")) // valid command for INSERTX
                    {
                        *total_client= *total_client-1;
                        printf("\none client closed\n");
                        close(client_id);
